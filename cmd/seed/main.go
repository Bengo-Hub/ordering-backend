...
func runSeed(ctx context.Context, client *ent.Client) (err error) {
	...
	tenantUUID, err := upsertTenant(ctx, tx)
	if err != nil {
		return err
	}
...
// upsertTenant ensures the default demo tenant exists by slug. Uses DB-generated UUID (no SetID) when creating.
// Returns the tenant UUID for use in the rest of the seed.
func upsertTenant(ctx context.Context, tx *ent.Tx) (uuid.UUID, error) {
	const (
		slug   = "urban-loft" // Must match auth-api and notifications-api default tenants
	...
	existing, err := tx.Tenant.Query().Where(tenant.SlugEQ(slug)).Only(ctx)
	if err == nil {
		return existing.ID, enqueueTenantSyncEvents(ctx, tx, existing.ID, slug)
	}
	if !ent.IsNotFound(err) {
		return uuid.Nil, fmt.Errorf("lookup tenant by slug: %w", err)
	}

	created, createErr := tx.Tenant.Create().
		SetSlug(slug).
		SetName(name).
		SetStatus(status).
		...
		Save(ctx)
	if createErr != nil {
		return uuid.Nil, fmt.Errorf("create tenant: %w", createErr)
	}
	return created.ID, enqueueTenantSyncEvents(ctx, tx, created.ID, slug)
}
