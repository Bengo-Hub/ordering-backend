// Package backup produces TENANT-SCOPED logical exports of ordering's own data. A backup
...
func (s *Service) Generate(ctx context.Context, tenantID uuid.UUID) (Info, error) {
	...
	name := fmt.Sprintf("ordering_backup_%s.json.gz", time.Now().UTC().Format("20060102_150405"))
