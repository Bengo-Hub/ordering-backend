...
// for the ordering backup module. The local PVC copy produced by the backup
...
// rest with AES-256-GCM using a Cipher (a SECRET_KEY-derived static key in this
// service), so secret backend params (access keys, tokens, passwords) are never
...
// and portable across services: services with a KeyProvider can pass that; this
// service passes a SECRET_KEY-derived static key.
...
type Destination struct {
	...
	// under (e.g. "ordering/backups", an S3 key prefix, or a Drive folder path).
