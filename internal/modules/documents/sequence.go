// Package documents provides per-tenant atomic document numbering for ordering-backend. It
// mirrors pos-api/inventory-api/treasury-api's DocumentSequence numbering exactly (same table
// shape, same CAS algorithm) — this platform's document-numbering pattern is duplicated
// per-service rather than a shared library, so this file is a deliberate copy of pos-api's
// internal/modules/documents/sequence.go (the closest analog: it mints POS order numbers the
// same way), not a divergent design. An optimistic compare-and-set on
// DocumentSequence.current_val yields race-safe, tenant-configurable document numbers. The
// platform default is PURE NUMERIC (e.g. "000001"); tenants opt into a prefixed/dated style
// (e.g. "ORD-260918-000001") per doc type in Settings.
package documents
...
import (
	...
	entdocseq "github.com/bengobox/ordering-backend/internal/ent/documentsequence"
)
...
// the zero-padded counter (e.g. "000001"), matching pos-api's own default for DocTypeOrder.
// Tenants who prefer the prefixed/dated style opt in per doc type in Settings → Documents, which
// sets a prefix and/or date_format.
var seqDefaults = map[string]seqConfig{
	DocTypeOrder: {Separator: "-", Padding: 6, ResetFreq: "never"},
}

// SuggestedPrefixes are the pre-fill hints a future Settings UI would offer when a tenant
// switches a doc type to the prefixed format. NOT applied automatically — the platform default
// is numeric.
var SuggestedPrefixes = map[string]string{
	DocTypeOrder: "ORD",
}

// configuredDocTypes is the set of ordering document types that carry a configurable sequence.
var configuredDocTypes = []string{DocTypeOrder}
...
// GetConfig returns the configuration (auto-seeding the default row if none exists yet) for one
// doc type — used by callers that need to branch on numeric-vs-prefix mode before minting.
...
// ListConfigs returns the configuration (auto-seeding defaults) for every ordering doc type.
