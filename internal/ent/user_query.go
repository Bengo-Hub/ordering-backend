...
type UserQuery struct {
	...
	withRiderProfile      *RiderProfileQuery
	withReviewedDocuments *RiderDocumentQuery
	withFKs               bool
...
func (uq *UserQuery) QueryPreferences() *UserPreferenceQuery {
	query := (&UserPreferenceClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryRiderProfile chains the current query on the "rider_profile" edge.
func (uq *UserQuery) QueryRiderProfile() *RiderProfileQuery {
	query := (&RiderProfileClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryReviewedDocuments chains the current query on the "reviewed_documents" edge.
func (uq *UserQuery) QueryReviewedDocuments() *RiderDocumentQuery {
	query := (&RiderDocumentClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withRiderProfile:      uq.withRiderProfile.Clone(),
		withReviewedDocuments: uq.withReviewedDocuments.Clone(),
...
func (uq *UserQuery) WithPreferences(opts ...func(*UserPreferenceQuery)) *UserQuery {
	query := (&UserPreferenceClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithRiderProfile tells the query-builder to eager-load the nodes that are connected to
// the "rider_profile" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithRiderProfile(opts ...func(*RiderProfileQuery)) *UserQuery {
	query := (&RiderProfileClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withRiderProfile = query
	return uq
}

// WithReviewedDocuments tells the query-builder to eager-load the nodes that are connected to
// the "reviewed_documents" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithReviewedDocuments(opts ...func(*RiderDocumentQuery)) *UserQuery {
	query := (&RiderDocumentClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withReviewedDocuments = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [11]bool{
			...
			uq.withProfile != nil,
			uq.withRiderProfile != nil,
			uq.withReviewedDocuments != nil,
		}
	)
	...
	if query := uq.withRiderProfile; query != nil {
		if err := uq.loadRiderProfile(ctx, query, nodes, nil,
			func(n *User, e *RiderProfile) { n.Edges.RiderProfile = e }); err != nil {
			return nil, err
		}
	}
	if query := uq.withReviewedDocuments; query != nil {
		if err := uq.loadReviewedDocuments(ctx, query, nodes,
			func(n *User) { n.Edges.ReviewedDocuments = []*RiderDocument{} },
			func(n *User, e *RiderDocument) { n.Edges.ReviewedDocuments = append(n.Edges.ReviewedDocuments, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadPreferences(ctx context.Context, query *UserPreferenceQuery, nodes []*User, init func(*User), assign func(*User, *UserPreference)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (uq *UserQuery) loadRiderProfile(ctx context.Context, query *RiderProfileQuery, nodes []*User, init func(*User), assign func(*User, *RiderProfile)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.RiderProfile(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.RiderProfileColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_rider_profile
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_rider_profile" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_rider_profile" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (uq *UserQuery) loadReviewedDocuments(ctx context.Context, query *RiderDocumentQuery, nodes []*User, init func(*User), assign func(*User, *RiderDocument)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*User)
	nids := make(map[uuid.UUID]map[*User]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.ReviewedDocumentsTable)
		s.Join(joinT).On(s.C(riderdocument.FieldID), joinT.C(user.ReviewedDocumentsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.ReviewedDocumentsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.ReviewedDocumentsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "reviewed_documents" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
