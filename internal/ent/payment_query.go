...
func (pq *PaymentQuery) QueryPaymentIntent() *PaymentIntentQuery {
	query := (&PaymentIntentClient{config: pq.config}).Query()
...
func (pq *PaymentQuery) Clone() *PaymentQuery {
	...
	return &PaymentQuery{
		...
		withPaymentIntent: pq.withPaymentIntent.Clone(),
...
func (pq *PaymentQuery) WithPaymentIntent(opts ...func(*PaymentIntentQuery)) *PaymentQuery {
	query := (&PaymentIntentClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withPaymentIntent = query
	return pq
}
...
func (pq *PaymentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Payment, error) {
	...
	if query := pq.withPaymentIntent; query != nil {
		if err := pq.loadPaymentIntent(ctx, query, nodes, nil,
			func(n *Payment, e *PaymentIntent) { n.Edges.PaymentIntent = e }); err != nil {
			return nil, err
		}
	}
...
func (pq *PaymentQuery) loadPaymentIntent(ctx context.Context, query *PaymentIntentQuery, nodes []*Payment, init func(*Payment), assign func(*Payment, *PaymentIntent)) error {
...
func (pq *PaymentQuery) loadRefunds(ctx context.Context, query *RefundQuery, nodes []*Payment, init func(*Payment), assign func(*Payment, *Refund)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(refund.FieldPaymentID)
	}
...
func (pq *PaymentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := pq.ctx.Fields; len(fields) > 0 {
		...
		if pq.withPaymentIntent != nil {
			_spec.Node.AddColumnOnce(payment.FieldPaymentIntentID)
		}
		if pq.withOrder != nil {
