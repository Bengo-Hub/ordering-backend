...
func (tq *TenantQuery) QuerySyncEvents() *TenantSyncEventQuery {
	query := (&TenantSyncEventClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TenantQuery) WithSyncEvents(opts ...func(*TenantSyncEventQuery)) *TenantQuery {
	query := (&TenantSyncEventClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withSyncEvents = query
	return tq
}
...
func (tq *TenantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tenant, error) {
	...
	if query := tq.withSyncEvents; query != nil {
		if err := tq.loadSyncEvents(ctx, query, nodes,
...
func (tq *TenantQuery) loadSyncEvents(ctx context.Context, query *TenantSyncEventQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *TenantSyncEvent)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Tenant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
