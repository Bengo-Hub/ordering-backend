...
func (_q *TenantQuery) QuerySettings() *TenantSettingQuery {
	query := (&TenantSettingClient{config: _q.config}).Query()
...
func (_q *TenantQuery) QuerySyncEvents() *TenantSyncEventQuery {
	query := (&TenantSyncEventClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *TenantQuery) WithSettings(opts ...func(*TenantSettingQuery)) *TenantQuery {
	query := (&TenantSettingClient{config: _q.config}).Query()
...
func (_q *TenantQuery) WithSyncEvents(opts ...func(*TenantSyncEventQuery)) *TenantQuery {
	query := (&TenantSyncEventClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TenantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tenant, error) {
	...
	if query := _q.withSyncEvents; query != nil {
		if err := _q.loadSyncEvents(ctx, query, nodes,
			func(n *Tenant) { n.Edges.SyncEvents = []*TenantSyncEvent{} },
			func(n *Tenant, e *TenantSyncEvent) { n.Edges.SyncEvents = append(n.Edges.SyncEvents, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TenantQuery) loadSettings(ctx context.Context, query *TenantSettingQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *TenantSetting)) error {
...
func (_q *TenantQuery) loadSyncEvents(ctx context.Context, query *TenantSyncEventQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *TenantSyncEvent)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Tenant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(tenantsyncevent.FieldTenantID)
	}
	query.Where(predicate.TenantSyncEvent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tenant.SyncEventsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
