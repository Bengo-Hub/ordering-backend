...
func (sq *SessionQuery) QueryTenant() *TenantQuery {
...
func (sq *SessionQuery) WithTenant(opts ...func(*TenantQuery)) *SessionQuery {
	query := (&TenantClient{config: sq.config}).Query()
...
func (sq *SessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Session, error) {
	...
	if query := sq.withTenant; query != nil {
		if err := sq.loadTenant(ctx, query, nodes, nil,
			func(n *Session, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
...
func (sq *SessionQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Session, init func(*Session), assign func(*Session, *Tenant)) error {
...
func (sq *SessionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := sq.ctx.Fields; len(fields) > 0 {
		...
		if sq.withTenant != nil {
			_spec.Node.AddColumnOnce(session.FieldTenantID)
		}
		if sq.withUser != nil {
