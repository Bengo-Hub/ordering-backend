...
import (
	...
	"github.com/bengobox/ordering-backend/internal/modules/backup/destination"
)
...
// SECRET_KEY-derived Cipher encrypts the destination config at rest.
...
func (h *BackupDestinationHandler) put(w http.ResponseWriter, r *http.Request, tenantID *uuid.UUID) {
	...
	if err := h.store.Store(r.Context(), tenantID, d); err != nil {
		...
		RespondError(w, http.StatusInternalServerError, "failed to save backup destination")
		return
	}
